...
v1 domain modules: `identity` (people/org), `roster`, `attendance`. Plus `controlplane` for tenant-level concerns and a `platform` package for cross-cutting infrastructure.
...
Request
  ...
  → Response
```
Control-plane operations (login, signup, tenant admin) use the control-plane pool directly and skip TenantMiddleware.
